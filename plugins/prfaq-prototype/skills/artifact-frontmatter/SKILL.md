...
The phase agent invokes this skill as the **first output step**, before writing any body section. **Order of operations:** compose the body *internally* first so the TL;DR, decisions-locked, and open `[VERIFY WITH PM]` items are derived from real content — then emit this block **first** (above the body) in the final artifact. The block is mechanically derived from the artifact/state — it is not hand-prose. Re-emit it whenever the artifact is regenerated (e.g., a v2 PRD after `[VERIFY]` resolution) so what-changed stays accurate.
