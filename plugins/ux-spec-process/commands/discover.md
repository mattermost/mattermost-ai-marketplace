...
- `specs/<slug>/spec-state.json` exists. **The state object is non-optional** (it is the run's memory + audit trail; running without one is the failure mode that left only 3/22 past specs tracked). If it is missing:
