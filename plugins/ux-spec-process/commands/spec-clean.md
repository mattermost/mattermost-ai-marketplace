...
4. **Once confirmed, delete the listed files.** Reinitialize via the mediated write path only — never hand-edit `spec-state.json` (Edit/Write are hook-denied; bash redirection / `sed` is prohibited):
   - First capture the prior `meta` fields (feature_name, created_at, author, tier) — e.g. `jq` them out before overwriting.
   - `cp ${CLAUDE_PLUGIN_ROOT}/templates/spec-state-object.json specs/<slug>/spec-state.json` (the one sanctioned file-creation step).
   - `${CLAUDE_PLUGIN_ROOT}/scripts/spec-state apply-delta <slug>` to restore the captured `meta` fields and set `phase.current` = 0, `phase.status` = "reset". **Do not pass `meta.last_updated` or any timestamp** — the CLI stamps `meta.last_updated`.
