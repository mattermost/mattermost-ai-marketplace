...
1. **Resolve slug.** Confirm with the user before creating any files.
...
4. **Initialize state object** at `specs/<slug>/spec-state.json`. First `cp ${CLAUDE_PLUGIN_ROOT}/templates/spec-state-object.json specs/<slug>/spec-state.json` — copying the template is the ONE sanctioned file-creation step. Then make **every** subsequent write through the `${CLAUDE_PLUGIN_ROOT}/scripts/spec-state` CLI; never hand-edit the file (a PreToolUse hook denies Edit/Write on it, and bash redirection / `sed` is prohibited):
