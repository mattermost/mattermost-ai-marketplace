...
```json
{
  ...
  "threats": [
    ...
    },
    {
      ...
      "recommended_mitigation": "DESIGN MITIGATION: When an admin creates a channel and sets classification level, show a warning: 'Messages in this channel can be marked with higher classification levels. Ensure ACL restricts membership to users with at least [SECRET] clearance if you expect [SECRET] content.' SYSTEM MITIGATION: Implement a 'max message classification' field on channels. Set it to [CONFIDENTIAL] by default, and require explicit admin choice if they want [SECRET] messages allowed. AUDIT MITIGATION: Log every message that is classified HIGHER than the channel's default. Security officers review these weekly.",
      "mitigation_effectiveness": "Warning + max_classification would significantly reduce misconfiguration risk. Requires admin discipline to read the warning.",
      "residual_risk": "Admin might ignore the warning. But audit logs would catch the issue quickly. Residual risk is Medium.",
