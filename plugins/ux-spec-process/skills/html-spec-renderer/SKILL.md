...
6. **Compose body** — assemble the HTML, inlining all token CSS, module CSS, and progressive-enhancement JS. **Pull all dynamic content from the manifest, never from the reference implementation.**
7. **Validate output (IL-honest gates)** — grep the emitted file for `cdn.`, `jsdelivr`, `unpkg`, `cdnjs`, `<script src`, `<link rel="stylesheet"`, `@import url`, `fetch(`, and any networked web font; confirm zero hits. Confirm light is the initial paint with no JS, the `<noscript>` block is present, every collapsible is native `<details>`, any diagram is inline `<svg>` (no runtime mermaid CDN), every badge pairs color+text, interactive elements carry ARIA + keyboard handlers, and every internal ID self-resolves. Any failure → do not write; emit `RENDER_FAIL`.
